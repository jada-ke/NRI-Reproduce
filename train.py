...
parser.add_argument('--use-mask', action='store_true', default=False,
                    help='Concatenate observation mask to encoder input and use masked loss.')
...
train_loader, valid_loader, test_loader, loc_max, loc_min, vel_max, vel_min = load_data(
    args.batch_size, args.suffix, use_mask=args.use_mask)
...
def train(epoch, best_val_loss):
    ...
    for batch_idx, (data, relations) in enumerate(train_loader):
        ...
        # Decoder only uses original feature dims (not mask channels)
        data_dec = data[:, :, :, :args.dims]
        if args.decoder == 'rnn':
            output = decoder(data_dec, edges, rel_rec, rel_send, 100,
                             burn_in=True,
                             burn_in_steps=args.timesteps - args.prediction_steps)
        else:
            output = decoder(data_dec, edges, rel_rec, rel_send,
                             args.prediction_steps)

        target = data_dec[:, :, 1:, :]

        if mask is not None:
            loss_nll = nll_gaussian_masked(output, target, args.var,
                                           mask[:, :, 1:, :])
        else:
    ...
    for batch_idx, (data, relations) in enumerate(valid_loader):
        ...
        output = decoder(data_dec, edges, rel_rec, rel_send, 1)
...
def test():
    ...
    for batch_idx, (data, relations) in enumerate(test_loader):
        ...
        data_feat = data[:, :, :, :args.dims]
        if args.decoder == 'rnn':
            if args.dynamic_graph:
                output = decoder(data_feat, edges, rel_rec, rel_send, 100,
            ...
            target = data_feat[:, :, -args.timesteps:, :]
        else:
            data_plot = data_feat[:, :, args.timesteps:args.timesteps + 21,
                        :].contiguous()
