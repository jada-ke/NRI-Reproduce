...
def mean_impute(arr):
    """Replace NaN with per-trajectory per-atom per-dim mean of observed values."""
    # arr: [sims, T, dims, atoms]
    obs_sum = np.nansum(arr, axis=1)                 # [sims, dims, atoms]
    obs_count = (~np.isnan(arr)).sum(axis=1)         # [sims, dims, atoms]
    obs_mean = obs_sum / np.maximum(obs_count, 1)    # [sims, dims, atoms]
    means = obs_mean[:, np.newaxis, :, :]            # [sims, 1, dims, atoms]
    return np.where(np.isnan(arr), means, arr).astype(np.float32)


def _load_mask(path, ref_shape):
    """Load a binary mask and broadcast to match feature shape [sims, T, dims, atoms].

    The dataset generator saves True=missing; we need 1=observed, 0=missing.
    """
    import os
    if not os.path.exists(path):
        return np.ones(ref_shape, dtype=np.float32)
    m = np.load(path).astype(np.float32)
    m = 1.0 - m  # flip: generator uses True=missing, we need 1=observed
    if m.ndim == 3:
        # [sims, T, atoms] -> [sims, T, 1, atoms] then broadcast to ref_shape
        m = np.expand_dims(m, axis=2)
        m = np.broadcast_to(m, ref_shape).copy()
    return m


def load_data(batch_size=1, suffix='', use_mask=False):
    loc_train = mean_impute(np.load('data/loc_train' + suffix + '.npy'))
    vel_train = mean_impute(np.load('data/vel_train' + suffix + '.npy'))
    edges_train = np.load('data/edges_train' + suffix + '.npy')

    loc_valid = mean_impute(np.load('data/loc_valid' + suffix + '.npy'))
    vel_valid = mean_impute(np.load('data/vel_valid' + suffix + '.npy'))
    edges_valid = np.load('data/edges_valid' + suffix + '.npy')

    loc_test = mean_impute(np.load('data/loc_test' + suffix + '.npy'))
    vel_test = mean_impute(np.load('data/vel_test' + suffix + '.npy'))
    ...
    if use_mask:
        mloc_tr = _load_mask('data/mask_loc_train' + suffix + '.npy', loc_train.shape)
        mvel_tr = _load_mask('data/mask_vel_train' + suffix + '.npy', vel_train.shape)
        mloc_va = _load_mask('data/mask_loc_valid' + suffix + '.npy', loc_valid.shape)
        mvel_va = _load_mask('data/mask_vel_valid' + suffix + '.npy', vel_valid.shape)
        mloc_te = _load_mask('data/mask_loc_test' + suffix + '.npy', loc_test.shape)
        mvel_te = _load_mask('data/mask_vel_test' + suffix + '.npy', vel_test.shape)
    ...
    if use_mask:
        # Transpose masks to [num_sims, num_atoms, num_timesteps, num_dims]
        mloc_tr = np.transpose(mloc_tr, [0, 3, 1, 2])
        mvel_tr = np.transpose(mvel_tr, [0, 3, 1, 2])
        mask_train = np.concatenate([mloc_tr, mvel_tr], axis=3)

        mloc_va = np.transpose(mloc_va, [0, 3, 1, 2])
        mvel_va = np.transpose(mvel_va, [0, 3, 1, 2])
        mask_valid = np.concatenate([mloc_va, mvel_va], axis=3)

        mloc_te = np.transpose(mloc_te, [0, 3, 1, 2])
        mvel_te = np.transpose(mvel_te, [0, 3, 1, 2])
        mask_test = np.concatenate([mloc_te, mvel_te], axis=3)

        # Encoder sees features + mask concatenated along last axis
        feat_train = np.concatenate([feat_train, mask_train], axis=3)
        feat_valid = np.concatenate([feat_valid, mask_valid], axis=3)
        feat_test = np.concatenate([feat_test, mask_test], axis=3)
...
def nll_gaussian_masked(preds, target, variance, mask, add_const=False):
    """NLL loss computed only on observed entries (mask=1)."""
    neg_log_p = ((preds - target) ** 2 / (2 * variance))
    if add_const:
        const = 0.5 * np.log(2 * np.pi * variance)
        neg_log_p += const
    neg_log_p = neg_log_p * mask
    return neg_log_p.sum() / (target.size(0) * target.size(1))
