...
class MaskedMLPEncoder(nn.Module):
    """MLPEncoder that aggregates only observed timesteps via masked mean+variance pooling.

    Instead of flattening all T timesteps (which propagates imputation artifacts),
    this encoder computes per-atom statistics (mean, variance) over observed
    timesteps only, eliminating false dynamics caused by missing-data imputation.
    """

    def __init__(self, n_in_feat, n_hid, n_out, do_prob=0., factor=True):
        # n_in_feat = dims (number of raw feature dims, e.g. 4 for pos+vel)
        super(MaskedMLPEncoder, self).__init__()
        self.factor = factor
        # Input to mlp1: mean + variance = 2 * n_in_feat
        self.mlp1 = MLP(2 * n_in_feat, n_hid, n_hid, do_prob)
        self.mlp2 = MLP(n_hid * 2, n_hid, n_hid, do_prob)
        self.mlp3 = MLP(n_hid, n_hid, n_hid, do_prob)
        if self.factor:
            self.mlp4 = MLP(n_hid * 3, n_hid, n_hid, do_prob)
            print("Using factor graph Masked MLP encoder.")
        else:
            self.mlp4 = MLP(n_hid * 2, n_hid, n_hid, do_prob)
            print("Using Masked MLP encoder.")
        self.fc_out = nn.Linear(n_hid, n_out)
        self.init_weights()
    ...
    def forward(self, inputs, rel_rec, rel_send):
        # inputs: [batch, atoms, T, 2*D]  feat=[:D], mask=[D:]
        D = inputs.size(3) // 2
        feat = inputs[:, :, :, :D]   # [batch, atoms, T, D]
        mask = inputs[:, :, :, D:]   # [batch, atoms, T, D]

        # Per-timestep binary flag: timestep is observed iff all its dims are observed
        obs = mask.min(dim=3, keepdim=True)[0]   # [batch, atoms, T, 1]

        # Masked mean over time
        n_obs = obs.sum(dim=2).clamp(min=1.0)    # [batch, atoms, 1]
        feat_mean = (feat * obs).sum(dim=2) / n_obs   # [batch, atoms, D]

        # Masked variance over time
        feat_sq_mean = ((feat ** 2) * obs).sum(dim=2) / n_obs
        feat_var = (feat_sq_mean - feat_mean ** 2).clamp(min=0)  # [batch, atoms, D]

        # Node representation: [batch, atoms, 2D]
        x = torch.cat([feat_mean, feat_var], dim=2)
...
class MaskedPairwiseEncoder(nn.Module):
    """
    Edge-centric masked encoder for missing data.

    For each edge (i,j), concatenates both atoms' features at each timestep and
    aggregates with masked mean+variance pooling over jointly-observed steps.
    This directly captures temporal co-variation between atom pairs, which is
    the signal NRI needs to infer spring/charge connections.
    """

    def __init__(self, n_in_feat, n_hid, n_out, do_prob=0., factor=True):
        super(MaskedPairwiseEncoder, self).__init__()
        self.factor = factor
        self.mlp1 = MLP(2 * n_in_feat, n_hid, n_hid, do_prob)
        ...
        if self.factor:
            self.mlp4 = MLP(n_hid * 3, n_hid, n_hid, do_prob)
            print("Using factor graph Masked Pairwise encoder.")
        else:
            self.mlp4 = MLP(n_hid * 2, n_hid, n_hid, do_prob)
            print("Using Masked Pairwise encoder.")
        self.fc_out = nn.Linear(n_hid, n_out)
        self.init_weights()
    ...
    def forward(self, inputs, rel_rec, rel_send):
        # inputs: [B, N, T, 2D]  feat cols [:D], mask cols [D:]
        B, N, T, D2 = inputs.size()
        D = D2 // 2
        E = rel_rec.size(0)

        feat = inputs[:, :, :, :D]   # [B, N, T, D]
        mask = inputs[:, :, :, D:]   # [B, N, T, D]

        # Per-atom, per-timestep observation flag (1 if all dims present)
        obs_node = mask.min(dim=3)[0]  # [B, N, T]

        # Gather sender/receiver features for every edge at every timestep
        feat_bt = feat.permute(0, 2, 1, 3).contiguous().view(B * T, N, D)
        send_feat = torch.matmul(rel_send, feat_bt).view(B, T, E, D).permute(0, 2, 1, 3)  # [B,E,T,D]
        recv_feat = torch.matmul(rel_rec,  feat_bt).view(B, T, E, D).permute(0, 2, 1, 3)  # [B,E,T,D]
        edge_feat = torch.cat([send_feat, recv_feat], dim=3)  # [B, E, T, 2D]

        # Edge mask: timestep valid iff BOTH endpoint atoms are observed
        obs_bt = obs_node.permute(0, 2, 1).contiguous().view(B * T, N, 1)
        send_obs = torch.matmul(rel_send, obs_bt).view(B, T, E).permute(0, 2, 1)  # [B, E, T]
        recv_obs = torch.matmul(rel_rec,  obs_bt).view(B, T, E).permute(0, 2, 1)  # [B, E, T]
        edge_obs = (send_obs * recv_obs).unsqueeze(3)  # [B, E, T, 1]

        # Per-timestep embedding of pairwise features
        # MLP.batch_norm expects 3D input [dim0, dim1, features]
        h = self.mlp1(edge_feat.contiguous().view(B * E, T, 2 * D)).view(B, E, T, -1)  # [B, E, T, n_hid]

        # Masked temporal aggregation: mean + variance over jointly-observed steps
        n_obs = edge_obs.sum(dim=2).clamp(min=1.0)     # [B, E, 1]
        h_mean = (h * edge_obs).sum(dim=2) / n_obs      # [B, E, n_hid]
        h_sq_mean = ((h ** 2) * edge_obs).sum(dim=2) / n_obs
        h_var = (h_sq_mean - h_mean ** 2).clamp(min=0)  # [B, E, n_hid]

        x = torch.cat([h_mean, h_var], dim=2)           # [B, E, 2*n_hid]
        x = self.mlp2(x)
        x_skip = x
