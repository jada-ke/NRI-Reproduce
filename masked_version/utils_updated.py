...
def load_data(batch_size=1, suffix='', use_mask=False):
    ...
    # Compute masks BEFORE normalization/filling (1=observed, 0=missing)
    # Mask is True wherever data is NOT NaN; broadcast across dims axis
    # loc/vel shape: [num_samples, num_timesteps, num_dims, num_atoms]
    # We use loc to determine missingness (loc and vel share the same mask)
    if use_mask:
        # Mask shape: [num_samples, num_timesteps, 1, num_atoms] -> broadcast over dims
        mask_train = (~np.isnan(loc_train[:, :, :1, :])).astype(np.float32)
        mask_valid = (~np.isnan(loc_valid[:, :, :1, :])).astype(np.float32)
        mask_test  = (~np.isnan(loc_test[:, :, :1, :])).astype(np.float32)
        # Expand mask to cover all dims so shape matches loc/vel
        mask_train = np.repeat(mask_train, loc_train.shape[2], axis=2)
        mask_valid = np.repeat(mask_valid, loc_valid.shape[2], axis=2)
        mask_test  = np.repeat(mask_test,  loc_test.shape[2],  axis=2)

    # # Fill NaNs with 0 before normalization (safe: 0 is within range)
        loc_train = np.nan_to_num(loc_train, nan=0.0)
        vel_train = np.nan_to_num(vel_train, nan=0.0)
        loc_valid = np.nan_to_num(loc_valid, nan=0.0)
        vel_valid = np.nan_to_num(vel_valid, nan=0.0)
        loc_test  = np.nan_to_num(loc_test,  nan=0.0)
        vel_test  = np.nan_to_num(vel_test,  nan=0.0)
    ...
    # Append mask channels: [num_sims, num_atoms, num_timesteps, num_dims*2]
    if use_mask:
        mask_train = np.transpose(mask_train, [0, 3, 1, 2])
        mask_valid = np.transpose(mask_valid, [0, 3, 1, 2])
        mask_test  = np.transpose(mask_test,  [0, 3, 1, 2])
