...
train_loader, valid_loader, test_loader, loc_max, loc_min, vel_max, vel_min = load_data(
    args.batch_size, args.suffix, use_mask=True)
...
encoder_input_dims = args.timesteps * (args.dims + 1)  # input dims includes mask feature

if args.encoder == 'mlp':
    encoder = MLPEncoder(encoder_input_dims, args.encoder_hidden,
                         args.edge_types,
                         args.encoder_dropout, args.factor)
elif args.encoder == 'cnn':
    encoder = CNNEncoder(args.dims * 2 if args.mask else args.dims, args.encoder_hidden,
                         args.edge_types,
                         args.encoder_dropout, args.factor)
...
def train(epoch, best_val_loss):
    ...
    for batch_idx, (data, relations) in enumerate(train_loader):
        ...
        data_vals = data[:, :, :, :args.dims]
        obs_mask = data[:, :, :, args.dims:args.dims+1]
        data_encoder_vals = data_vals * obs_mask # encoder only uses observed values

        # Rebuild encoder input
        encoder_input = torch.cat([data_encoder_vals, obs_mask], dim=-1)

        # Use masked input in encoder
        logits = encoder(encoder_input, rel_rec, rel_send)
        edges = gumbel_softmax(logits, tau=args.temp, hard=args.hard)
        prob = my_softmax(logits, -1)

        # Decoder still uses original (forward-filled) values
        data_decoder = data_vals
        ...
        target_mask = obs_mask[:, :, 1:, :]

        #masked loss
        loss_nll = nll_gaussian_masked(output, target, args.var, target_mask)
    ...
    for batch_idx, (data, relations) in enumerate(valid_loader):
        ...
        with torch.no_grad():
            data_vals = data[:, :, :, :args.dims]
            obs_mask = data[:, :, :, args.dims:args.dims+1]
            data_encoder_vals = data_vals * obs_mask
...
def test():
    ...
    with torch.no_grad():
        for batch_idx, (data, relations) in enumerate(test_loader):
            ...
            data_vals = data_encoder[:, :, :, :args.dims]
            obs_mask = data_encoder[:, :, :, args.dims:args.dims+1]
            data_encoder_vals = data_vals * obs_mask
            ...
            # decoder stays unchanged
            data_decoder = data[:, :, -args.timesteps:, :args.dims].contiguous()
            obs_mask = data[:, :, -args.timesteps:, args.dims:args.dims+1].contiguous()
